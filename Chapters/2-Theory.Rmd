...
Multivariate data imputation has the added benefit of being able to exploit dependencies between covariates when imputing missing data, as variables are often related in the real world ...
\todo[inline]{Work on making this sentence not as shit}
...
\end{equation}
To impute a missing data point we consider the mean value of the K nearest neighbours by Euclidean distance. This approach works well for the correct choice of $k$, but the downside is that this is a hyper-parameter and has to be chosen, perhaps by cross-validation. Another downside is that KNN requires the entire dataset to be stored in memory, which can be infesible for large datasets.
