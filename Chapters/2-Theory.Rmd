...
is a $d$ dimensional vector. In this case each the time series consists of an ordered sequence of $t$ observations of several varianbles. We have for each observation $x_i = \{x_{i1}, x_{i2}, ..., x_{id}\}$ is a vector of $d$ values at each time point.

## Missing data Mechanisms

Missing data is a common occurrence and can be due to human error, or just unforeseeable. Missing data can arise from device failures such as
measuring equipment failing, or from data censoring (such as from
...
\end{equation}
where $r$ is the data missingness indicator variable [@twumasi2019efficiency].  
...
P(r|Y_{observed},Y_{missing}) = P(r|Y_{missing}).
\end{equation}
...
The data imputation methods considered for our project work best under the assumptions of MCAR or MAR. This is because, under these mechanisms, the missingness is not informative towards the missing values themselves. These methods may not be suitable in the case of NMAR.
...
Last Observed Carried Forward (LOCF) is a simple imputation method were a missing data point $x_j$ is replaced with the previous value $x_{j-1}$. This imputation method does not require any assumptions on the series, but does work best for series where the underlying process is stable or changes slowly overtime. It also work best when there are not several consecutive missing values as this would create a long period where the last non missing value is repeated. If in impute a missing value $x_j$ with the next value $x_{j+1}$ this is called Next observed carried backward (NOCB).
...
Linear interpolation uses the two non missing values on the left and right of a missing value in order to impute it. I use the equation as used in [@lepot2017interpolation] by the equation
...
The Kalman filter was introduced in 1960 by Rudolph Emil Kalman. Kalman filtering is used in a state space model. The kalman filter is an algorithm that works within a state space model framework. It consists of a state equation which models the evolution of the system overtime, and a measurement equation which describes the relationship between the system state to the observed noisey measurements. A Kalman filter is efficient and optimal estimator in terms of minimum mean squared error (MMSE), if the series has both Gaussian noise and the system exhibits linear behavior.
When used for missing data the Kalman filter uses the current state transition equation to predict the missing value. When observed data is encountered it will update the state estimate. The assumptions make for a Kalman filter to be effective are that the system should be linear with Gaussian noise (if it this is not the case then the the extended Kalman filter would be more appropriate [@benrhmach2020nonlinear]).


### Multivariate data imputation techniques

In a univariate time series there is only the observation and the time point, so in general univariate data imputation is more difficult. With a multivariate time series there are now several variables that relate to an observation at that time point. In many real world scenarios there are correlations between the predictors that can be exploited to fill in a missing value.
