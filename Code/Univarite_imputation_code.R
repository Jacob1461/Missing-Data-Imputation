...
data_frames <- list(data.frame(), data.frame(), data.frame(), data.frame(), data.frame(), data.frame(), data.frame(), data.frame(), data.frame(), data.frame())
for(i in 1:N_times){
  set.seed(seeds[i])
  # sunspots
...
df_long$Method <- recode(df_long$Method, 
                         locf_rmse = "LOCF",
                         mean_rmse = "Mean",
                         linear_rmse = "Linear Interpolation",
                         kalman_rmse = "Kalman")



p <- ggplot(df_long, aes(x = x, y = RMSE, color = Method)) +
  geom_line(size = 1) +  # Add lines for each method
  scale_color_manual(values = c("black", "red", "green", "blue")) +  # Set manual colors
  labs(title = "Sunspots Dataset Imputation Methods",
       x = "Missingness percent",  # You can modify the x and y axis labels
       y = "RMSE") +
  theme_minimal() +  # Use a clean theme
  theme(plot.title = element_text(hjust = 0.5))  # Center the title

ggsave("graphs/univariate/sunspots.png", plot = p, width = 6, height = 4, dpi = 300, bg = "white")
...
p <- ggplot(df_long, aes(x = x, y = RMSE, color = Method)) +
  geom_line(size = 1) +  # Add lines for each method
  scale_color_manual(values = c("black", "red", "green", "blue")) +  # Set manual colors
  labs(title = "LakeHuron Dataset Imputation Methods",
       x = "Missingness percent",  # You can modify the x and y axis labels
       y = "RMSE") +
  theme_minimal() +  # Use a clean theme
  theme(plot.title = element_text(hjust = 0.5))  # Center the title

ggsave("graphs/univariate/lake.png", plot = p, width = 6, height = 4, dpi = 300, bg = "white")
...
p <- ggplot(df_long, aes(x = x, y = RMSE, color = Method)) +
  geom_line(size = 1) +  # Add lines for each method
  scale_color_manual(values = c("black", "red", "green", "blue")) +  # Set manual colors
  labs(title = "BeerSales Dataset Imputation Methods",
       x = "Missingness percent",  # You can modify the x and y axis labels
       y = "RMSE") +
  theme_minimal() +  # Use a clean theme
  theme(plot.title = element_text(hjust = 0.5))  # Center the title

ggsave("graphs/univariate/beer.png", plot = p, width = 6, height = 4, dpi = 300, bg = "white")
...
p <- ggplot(df_long, aes(x = x, y = RMSE, color = Method)) +
  geom_line(size = 1) +  # Add lines for each method
  scale_color_manual(values = c("black", "red", "green", "blue")) +  # Set manual colors
  labs(title = "Lynx Dataset Imputation Methods",
       x = "Missingness percent",  # You can modify the x and y axis labels
       y = "RMSE") +
  theme_minimal() +  # Use a clean theme
  theme(plot.title = element_text(hjust = 0.5))  # Center the title
